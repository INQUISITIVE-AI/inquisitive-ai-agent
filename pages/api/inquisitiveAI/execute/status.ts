...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  const nextAction: Record<Level, string> = {
    ...
    AUTOMATION_ACTIVE:  'Deploy the Chainlink Functions OracleConsumer per chainlink-functions/SETUP_GUIDE.md (forge script script/DeployOracleConsumer.s.sol), wire a Functions subscription + Automation upkeep, then vault.setAIOracle(<consumer>).',
    FULLY_OPERATIONAL:  'Vault configured. Chainlink Automation calls OracleConsumer.performUpkeep() → Functions DON fetches signals → OracleConsumer.fulfillRequest() → vault.submitSignalsBatch() → vault.performUpkeep() executes Uniswap V3 swaps.',
  };
